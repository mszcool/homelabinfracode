...
# Provision Ring 1 VMs with Terraform
cd terraform
...
- **Deployment**: Container on k3s or standalone VM
...
1. **VMs are provisioned via Terraform** using `ring1.tfvars` — define the VM in the Terraform configuration, then `terraform apply`
2. **Service configuration uses Ansible** — create a playbook in `playbooks/ring1/` that configures the service after VM provisioning
3. **Depend only on Ring 0**: Use Incus for compute, TrueNAS for persistent storage, MikroTik for network/DNS, Samba4 for identity
4. **Never create Ring 0 dependencies on Ring 1**: If a Ring 1 service needs DNS, it gets entries from the router (Ring 0), not from a Ring 1 DNS service
